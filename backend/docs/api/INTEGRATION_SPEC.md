...
### 2.5b Endpoint: Cancel Order

Both the dispatcher and driver can cancel an order that is still `pending` or `assigned`.

```
POST /api/v1/orders/:orderId/cancel
```

**Path Parameter**: `orderId` — Order document ID (Firestore ID)
...
**Cancellation Rules**:
- Only orders with backend status `pending` or `assigned` can be cancelled.
- Canceled orders get `status: "failed"`, `driverStatus: "failed"`, and `failureReason` set to the provided reason.
- The order is removed from the assigned courier's `assignedOrders` array.
- An entry is written to `ai_decision_logs` with the cancellation details.
